...
- `apps/web` — SvelteKit (Svelte 5 runes) + Tailwind v4 + adapter-cloudflare, deployed as a Cloudflare Worker with static assets. Design tokens live in `src/routes/layout.css` (`@theme`). Adapter is configured in `vite.config.ts` (no svelte.config.js in this template); its `platformProxy.persist` points at `apps/api/.wrangler` so dev shares one local D1. Auth: better-auth in `src/lib/server/auth.ts` (`getAuth(env)`, memoized per isolate), mounted in `hooks.server.ts` via `svelteKitHandler`; `locals.user`/`locals.session` populated there. Auth tables live in `packages/db/src/auth-schema.ts` (generated by `@better-auth/cli`, English names); `estilistas.user_id` links to `user.id`. Secrets/vars: `BETTER_AUTH_SECRET` (Wrangler secret / `.dev.vars`); `API_BASE_URL` (wrangler var, the api Worker URL) and `MOCK_CHAT_SECRET` (shared with the api) power the `/app/probar-agente` browser test chat — its `+page.server.ts` action `fetch`es `POST /mock/chat` server-to-server. Plan changes go through `/app/plan` → `/app/plan/checkout` (mocked Webpay-style pago that always approves): the `pagar` action writes a `suscripciones` row and sets `estilistas.tier_id` (all gating reads off that FK, so the agent unlocks immediately). **Admin panel** at `/admin` (own route group + `+layout.server.ts` guard via `esAdmin`, checks `user.role === 'admin'`): accounts table with tier/estado/waEstado/monthly-cost, change tier, pause/reactivate. Promote an account: `wrangler d1 execute tuhorafacil --local --command "UPDATE user SET role='admin' WHERE email='…'"` (`:remote` for prod). **`/admin/prompt`** is an admin-only editor for the global agent system prompt (stored in `configuracion`, clave `system_prompt`; empty row = default); tier changes are NOT admin-only — estilistas self-serve via `/app/plan`. **Profile photos** use a private R2 bucket (binding `FOTOS`, bucket `tuhorafacil-fotos` — create with `wrangler r2 bucket create` for prod); uploaded via the `foto` action in `/app/pagina` (R2 `put` needs `arrayBuffer()`, not `File.stream()`), served by `routes/fotos/[...key]/+server.ts`, stored as `estilistas.foto_url`.
- `apps/api` — Hono on Cloudflare Workers. `wrangler types` generates `Env` (run via `pnpm check`). D1 binding `DB` (database `tuhorafacil`), migrations_dir points at `packages/db/drizzle`. WhatsApp webhook at `/webhook/whatsapp` (GET verify + POST with `waitUntil`); the agent lives in `src/agente/` (orquestador → 5 strict tools, model `claude-haiku-4-5`, manual tool loop ≤6 iterations). The **system prompt template lives in `packages/agenda/src/prompt.ts`** (`construirSystemPrompt` + exported `PLANTILLA_POR_DEFECTO`, with `{{tokens}}` for runtime data) so both api and the web admin editor share it; the orquestador loads an admin override from `configuracion` (clave `system_prompt`) or falls back to the default. Secrets: `WA_VERIFY_TOKEN`, `ANTHROPIC_API_KEY`, `WA_ACCESS_TOKEN` (all optional-degrading: no API key → fallback reply + conversation escalada; no WA token → sends are logged). Coexistence cooldown via `conversaciones.agente_pausado_hasta`. `registrarConsumo` also writes `consumo_mensual.costo_estimado_usd` (Haiku 4.5 pricing constants live next to `MODELO`) — shown in the admin panel. The seed demo estilista has `wa_phone_number_id = 'DEMO_PHONE_ID'`, so you can exercise the real webhook by POSTing a hand-built WhatsApp payload with that phone id. **Browser test channel:** `POST /mock/chat` (+ `/mock/chat/reset`), guarded by header `X-Mock-Secret === MOCK_CHAT_SECRET`, reuses the exact agent core (`correrAgente`) against a synthetic `WEB_DEMO` clienta — applies the same tier/activo/límite gates but skips `waEstado`, so no WhatsApp is needed (`src/agente/simular.ts`, `src/routes/mock.ts`).
- `packages/agenda` — shared domain logic (crearCita atomic insert, `editarCita` atomic in-place update that preserves `origen` and excludes its own row via `citasDelDia(…, excluirCitaId)`, bloqueos, slots, fechas Chile TZ) used by both web and the agent. Never duplicate this logic in an app.
