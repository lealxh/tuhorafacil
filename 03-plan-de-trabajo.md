...
| **Recordatorios (F7) — infra + vista de prueba** | Cron diario (14:00 UTC) en la api genera recordatorios para las citas de mañana de cuentas Pro (tabla `recordatorios`, idempotente: unique por cita). Sin envío real todavía: quedan `estado='simulado'` y se ven en `/app/recordatorios` (gate por `tiene_recordatorios`) renderizados como los verá la clienta en WhatsApp, con botones Confirmar/Reagendar que simulan su respuesta (`respuesta`/`respondido_at`). `POST /mock/recordatorios` (X-Mock-Secret) los genera al instante para probar. Al aprobar Meta: enchufar plantilla con quick-replies en el cron y capturar la respuesta real por webhook | migraciones 0007/0008; `apps/api/src/recordatorios/generar.ts` + cron en `index.ts`; `apps/web/.../recordatorios` |
