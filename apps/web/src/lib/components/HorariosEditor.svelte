<script lang="ts">
	...
	let { horarios }: { horarios: { diaSemana: number; horaInicio: string; horaFin: string }[] } = $props();

	const horarioDe = (dia: number) => horarios.find((h) => h.diaSemana === dia);
	// Sin datos aún: propone lunes a viernes abierto
	const abiertoDefault = (dia: number) => (horarios.length === 0 ? dia >= 1 && dia <= 5 : horarioDe(dia) !== undefined);
</script>

{#each ORDEN as dia (dia)}
	<div class="rounded-field flex flex-col gap-2.5 bg-white px-4 py-3 shadow-sm">
		<label class="flex items-center gap-2.5">
			<input type="checkbox" name="abierto-{dia}" checked={abiertoDefault(dia)} class="text-primary focus:ring-primary/30 rounded" />
