<script lang="ts">
	...
	const horaDesde = $derived(
		data.horarios.length ? Math.floor(Math.min(...data.horarios.map((h) => aMinutos(h.horaInicio))) / 60) : 9
	);
	const horaHasta = $derived(
		data.horarios.length ? Math.ceil(Math.max(...data.horarios.map((h) => aMinutos(h.horaFin))) / 60) : 20
	);
	...
	const diasSemana = $derived(
		Array.from({ length: 7 }, (_, i) => {
			...
			return { fecha, num: Number(fecha.slice(8)), citas: data.citas.filter((c) => c.fecha === fecha) };
		})
	);
		...
		<div class="px-6">
			...
			<div class="relative" style="height:{altoDia}px">
				{#each Array.from({ length: horaHasta - horaDesde + 1 }, (_, i) => horaDesde + i) as hora (hora)}
					<div class="absolute inset-x-0 h-px bg-[#EFE0D9]" style="top:{(hora - horaDesde) * 54}px"></div>
					<p class="text-ink-faint absolute left-0 text-[10.5px] font-semibold" style="top:{(hora - horaDesde) * 54 - 6}px">
				...
				{#each data.bloqueos as bloqueo (bloqueo.id)}
					<button
						...
						style="top:{topDe(bloqueo.horaInicio)}px;height:{Math.max((aMinutos(bloqueo.horaFin === '24:00' ? '23:59' : bloqueo.horaFin) - aMinutos(bloqueo.horaInicio)) * PX_POR_MIN, 20)}px"
					>
						<p class="text-ink-soft text-[12px] font-semibold">🔒 {bloqueo.motivo ?? 'Bloqueado'}</p>
					</button>
				{/each}
				{#each data.citas as cita (cita.id)}
					<button
						...
						style="top:{topDe(cita.horaInicio)}px;height:{Math.max((aMinutos(cita.horaFin) - aMinutos(cita.horaInicio)) * PX_POR_MIN, 24)}px"
					>
						<p class="text-[13px] font-semibold">{cita.clienta}</p>
						<p class="text-ink-soft text-[11.5px]">{cita.servicio} · {cita.horaInicio}–{cita.horaFin}</p>
					</button>
				{/each}
			</div>
		</div>
			...
			<div class="flex gap-1.5">
				{#each diasSemana as dia (dia.fecha)}
					<a href={link('dia', dia.fecha)} class="flex flex-1 flex-col items-center gap-1.5">
						<span class="text-ink-soft text-[10.5px] font-semibold">{DOWS[(diaSemanaDe(dia.fecha) + 6) % 7]}</span>
						<span
							class="flex h-8 w-8 items-center justify-center rounded-full text-[13px] font-bold {dia.fecha === data.hoy
						...
						<span class="mt-0.5 flex w-full flex-col gap-1">
							{#each dia.citas as cita (cita.id)}
								<span
									...
									style="height:{Math.max((aMinutos(cita.horaFin) - aMinutos(cita.horaInicio)) * 0.3, 8)}px"
								></span>
							{/each}
						</span>
					</a>
					...
					{#if celda}
						<a
							...
							class="flex aspect-square flex-col items-center justify-center gap-0.5 rounded-[11px] {celda.fecha === data.hoy
								...
								: 'bg-white/60'}"
						>
							<span class="text-xs {celda.fecha === data.hoy ? 'text-primary font-bold' : 'font-semibold'}">{celda.dia}</span>
		...
		<div class="bg-background w-full rounded-t-[28px] px-6 pt-2.5 pb-8">
			...
			{#if sheet === 'nueva'}
				...
				<form method="POST" action="?/crear" use:enhance={alGuardar} class="mt-4 flex flex-col gap-3">
					...
					<label class="flex flex-col gap-1.5">
						...
						<input name="telefono" type="tel" required placeholder="+56 9 8765 4321" class={inputSheet} />
					</label>
					<label class="flex flex-col gap-1.5">
					...
					<div class="flex gap-2.5">
						...
						<label class="flex min-w-0 flex-1 flex-col gap-1.5">
							...
							<input name="horaInicio" type="time" required value="12:00" step="900" class={inputSheet} />
						</label>
					</div>
					{#if form && 'sheet' in form && form.sheet === 'crear'}<p class="text-blush-deep bg-blush rounded-field px-4 py-2.5 text-sm">{form.error}</p>{/if}
					<button
						type="submit"
					...
					<button type="button" onclick={() => (sheet = 'bloquear')} class="text-ink-soft py-1 text-sm font-semibold">
						o bloquear un horario 🔒
					</button>
				</form>
			{:else if sheet === 'bloquear'}
				<h2 class="text-xl font-bold tracking-tight">Bloquear horario</h2>
				<form method="POST" action="?/bloquear" use:enhance={alGuardar} class="mt-4 flex flex-col gap-3">
					<label class="flex flex-col gap-1.5">
						<span class="text-ink-soft text-xs font-semibold">Fecha</span>
						<input name="fecha" type="date" required value={data.fecha} class={inputSheet} />
					</label>
					...
					{#if form && 'sheet' in form && form.sheet === 'bloquear'}<p class="text-blush-deep bg-blush rounded-field px-4 py-2.5 text-sm">{form.error}</p>{/if}
					<button
						type="submit"
			...
			{:else if citaSel}
				...
				<p class="text-ink-soft mt-1 text-sm">
					...
					{#if citaSel.origen === 'agente'}<span class="bg-blush text-primary ml-1 rounded-full px-2 py-0.5 text-[10px] font-bold">✨ agente</span>{/if}
				</p>
				<form method="POST" action="?/cancelar" use:enhance={alGuardar} class="mt-5">
			...
			{:else if bloqueoSel}
				<h2 class="text-xl font-bold tracking-tight">🔒 {bloqueoSel.motivo ?? 'Horario bloqueado'}</h2>
				...
				<form method="POST" action="?/desbloquear" use:enhance={alGuardar} class="mt-5">
					...
					<button type="submit" class="rounded-field border-line text-ink-soft w-full border bg-white px-4 py-3 text-sm font-bold">
						Quitar bloqueo
					</button>
				</form>
			{/if}
