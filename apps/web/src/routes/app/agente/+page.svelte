...
<main class="flex flex-col gap-4 p-6 lg:p-8">
	...
	{#if !data.tieneAgente}
		<div class="rounded-card flex flex-col items-center gap-3 bg-white p-8 text-center shadow-sm">
			...
			<p class="text-ink-soft text-sm">
				Con el plan <strong>Recepcionista</strong> tu agente responde WhatsApp y agenda citas mientras trabajas.
			</p>
	...
	{:else}
		...
		>
			<div
				class="flex h-11 w-11 items-center justify-center rounded-2xl text-xl {activo && waConectado
					? 'bg-white/20'
					: 'bg-blush'}"
			>
				{activo && waConectado ? '✨' : '😴'}
			...
			<div class="mt-2.5 flex gap-2">
				{#each PERSONALIDADES as [valor, etiqueta] (valor)}
					<form method="POST" action="?/personalidad" use:enhance class="flex-1">
						...
						<button
							type="submit"
							class="rounded-field w-full border py-2.5 text-[13px] font-semibold transition {(data.config?.personalidad ?? 'cercana') === valor
		...
		<!-- Instrucciones -->
		<form method="POST" action="?/instrucciones" use:enhance class="rounded-card flex flex-col gap-3 bg-white p-4 shadow-sm">
			<label class="flex flex-col gap-1.5">
				<span class="text-ink-soft text-xs font-bold tracking-wider uppercase">Instrucciones para tu agente</span>
				<textarea
					name="instrucciones"
					rows="3"
					placeholder="Ej: Tuteo chileno. Pedir abono del 30% para balayage. No agendar después de las 18:00 los viernes."
					class="rounded-field border-line focus:border-primary bg-surface border px-3.5 py-2.5 text-sm">{data.config?.instrucciones ?? ''}</textarea>
			</label>
			<label class="flex flex-col gap-1.5">
				<span class="text-ink-soft text-xs font-bold tracking-wider uppercase">Información extra</span>
				<textarea
					name="infoExtra"
					rows="2"
					placeholder="Ej: link de pago, cómo llegar, estacionamiento…"
					class="rounded-field border-line focus:border-primary bg-surface border px-3.5 py-2.5 text-sm">{data.config?.infoExtra ?? ''}</textarea>
			</label>
		...
		<div class="flex items-center justify-between">
			<h2 class="text-ink-soft text-xs font-bold tracking-wider uppercase">Escalados pendientes</h2>
			{#if data.escaladas.length}
				<span class="bg-primary-light rounded-full px-2 py-0.5 text-[11px] font-bold text-white">{data.escaladas.length}</span>
			{/if}
		</div>
	...
	>
		<div class="bg-background w-full rounded-t-[28px] px-6 pt-2.5 pb-8 lg:max-w-md lg:rounded-[28px] lg:pb-6">
			...
			<div class="mt-5 flex gap-3">
				<form method="POST" action="?/seguirAgente" use:enhance={() => async ({ update }) => { escaladaSel = null; await update(); }} class="flex-1">
				...
				<form method="POST" action="?/responderYo" use:enhance={() => async ({ update }) => { escaladaSel = null; await update(); }} class="flex-1">
					<input type="hidden" name="id" value={escaladaSel.id} />
					<button
						type="submit"
