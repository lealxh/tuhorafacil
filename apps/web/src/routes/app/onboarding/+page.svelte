			...
			{#each data.servicios as servicio (servicio.id)}
				<div class="rounded-field flex items-center gap-3 bg-white px-4 py-3 shadow-sm">
					...
					<form method="POST" action="?/eliminarServicio" use:enhance>
						...
						<button type="submit" aria-label="Eliminar {servicio.nombre}" class="text-ink-faint text-lg">✕</button>
					</form>
				</div>
			{:else}
				<p class="text-ink-soft rounded-field bg-blush px-4 py-3 text-sm">Agrega tu primer servicio 👇</p>
			{/each}
		</div>

		<form method="POST" action="?/agregarServicio" use:enhance class="rounded-card flex flex-col gap-3 bg-white p-4 shadow-sm">
			...
			<button type="submit" class="rounded-field bg-blush text-blush-deep px-4 py-2.5 text-sm font-bold">+ Agregar servicio</button>
		</form>

		{#if form?.error}<p class="text-blush-deep bg-blush rounded-field px-4 py-2.5 text-sm">{form.error}</p>{/if}

		<div class="mt-auto flex gap-3">
			<a href="/app/onboarding?paso=1" class="rounded-field border-line text-ink-soft flex-1 border bg-white px-4 py-3 text-center text-sm font-semibold">Atrás</a>
			{#if data.servicios.length > 0}
				<a href="/app/onboarding?paso=3" class="{btnPrimario} flex-1 text-center">Continuar</a>
			{/if}
		</div>
	{:else if data.paso === 3}
		...
		<form method="POST" action="?/guardarHorarios" use:enhance class="flex flex-1 flex-col gap-2.5">
			...
			<div class="mt-auto flex gap-3 pt-3">
				<a href="/app/onboarding?paso=2" class="rounded-field border-line text-ink-soft flex-1 border bg-white px-4 py-3 text-center text-sm font-semibold">Atrás</a>
	...
	{:else}
		...
		<div class="rounded-card flex flex-col items-center gap-3 bg-white p-8 text-center shadow-sm">
			...
			<p class="text-ink-soft text-sm">
				La conexión con WhatsApp se activa cuando Meta apruebe la plataforma. Tu agenda ya queda lista.
			</p>
		</div>

		<div class="mt-auto flex gap-3">
			<a href="/app/onboarding?paso=3" class="rounded-field border-line text-ink-soft flex-1 border bg-white px-4 py-3 text-center text-sm font-semibold">Atrás</a>
			<a href="/app" class="{btnPrimario} flex-1 text-center">Ir a mi agenda</a>
		</div>
	{/if}
</main>
